...
class SklearnModel(BaseEstimator, RegressorMixin):
    ...
    def __init__(self,
                 ...
                 nu: float = 3,
                 q: float = 0.9,
                 over_estimate_method: str = "sample_std",
                 fit_sigma_prior: bool = True,
                 n_samples: int = 1000,
                 n_burn: int = 200,
                 thin: float = 0.1,
                 alpha: float = 0.95,
        ...
        self.nu = nu
        self.q = q
        self.over_estimate_method = over_estimate_method
        self.fit_sigma_prior = fit_sigma_prior
        self.n_burn = n_burn
        self.n_samples = n_samples
        self.p_grow = p_grow
        self.p_prune = p_prune
        self.p_change = p_change
        self.tree_sampler = get_tree_sampler(p_grow, p_prune, p_change)
        self.alpha = alpha
        self.beta = beta
        self.Gamma = Gamma
        self.lam = lam
    ...
    def _set_sigma_a_and_sigma_b(self, X: np.ndarray, y: np.ndarray):
        ...
        self.sigma_a = self.nu / 2
        self.sigma_b = (self.nu * lam) / 2
    ...
    def _construct_model(self, X: np.ndarray, y: np.ndarray) -> Model:
        ...
        if self.fit_sigma_prior:
            self._set_sigma_a_and_sigma_b(X, y)
        self.c = self.c*np.log(X.shape[0])
    ...
    def out_of_sample_credible_set_and_mean(self, X, gamma):
        alpha = 1 - gamma
        predictions = [x.predict(X) for x in self._model_samples]
        lower = self.data.unnormalize_y(np.quantile(predictions,
                                                    alpha / 2, axis=0))
        higher = self.data.unnormalize_y(np.quantile(predictions,
                                                     1 - alpha / 2, axis=0))
        mean = self.data.unnormalize_y(np.mean(predictions, axis=0))
        return lower, higher, mean

    def produce_rmse(self, X, y, gamma):
        lower, higher, mean = self.out_of_sample_credible_set_and_mean(X, gamma)
        lower = np.sqrt(np.sum(np.square(y - lower)))
        higher = np.sqrt(np.sum(np.square(y - higher)))
        mean = np.sqrt(np.sum(np.square(y - mean)))
        return lower, higher, mean


    def out_of_sample_prediction_interval(self, X, gamma):
        alpha = 1 - gamma
        sigma_samples = np.array([x.sigma.current_value() for x in self._model_samples]).reshape((len(self._model_samples), 1))
        predictions = np.array([x.predict(X) for x in self._model_samples]) + np.random.normal(scale=sigma_samples)
        lower = self.data.unnormalize_y(np.quantile(predictions, alpha / 2, axis=0))
        higher = self.data.unnormalize_y(np.quantile(predictions, 1 - alpha / 2, axis=0))
        return lower, higher
